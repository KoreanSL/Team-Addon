...
*/ 
function tag(player, tag){let arr = [];tag.forEach(x => arr.push(player.hasTag(x)));return arr.includes(true)}import{world}from"mojang-minecraft";let team = ["team:none","team:red","team:blue","team:green","team:yellow","team:aqua","team:white","team:pink","team:gray"];let color = ["","§c","§9","§a","§e","§b","§f","§d","§7"];world.events.tick.subscribe(()=>{for(let player of world.getPlayers()){console.warn(player.nameTag);try{if(tag(player, team)==false)player.addTag("team:none");else if(tag(player, ["team:red","team:blue","team:green","team:yellow","team:aqua","team:white","team:pink","team:gray"])==true)player.removeTag("team:none");}catch{};for(let i = 0; i<team.length; i++)if(player.hasTag(team[i]))player.nameTag = "§f[ " + color[i]+team[i].slice(5) + "§f ] " + player.name}});world.events.beforeChat.subscribe(data => {if(data.message)for(let i = 0; i<team.length; i++)if(data.sender.hasTag(team[i])){data.cancel = true;world.getDimension("overworld").runCommand(`tellraw @a {"rawtext":[{"text":"§f[ ${color[i]}${team[i].slice(5)}§f ] ${data.sender.name} > ${data.message}"}]}`)}});world.events.tick.subscribe(() => {for(let player of world.getDimension("overworld").getPlayers())if(player.hasTag("team:reset"))for(let i = 0; i<["team:red","team:blue","team:green","team:yellow","team:aqua","team:white","team:pink","team:gray"].length; i++)player.removeTag(["team:red","team:blue","team:green","team:yellow","team:aqua","team:white","team:pink","team:gray"][i])})